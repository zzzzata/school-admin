$(function () {
    $("#jqGrid").jqGrid({
        url: '../mallmarketcourse/classifyList',
        datatype: "json",
        colModel: [		
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			if($.isNull(vm.mallMarketCourseClassify.name)){
				alert("请输入名称！！！");
				return;
			}
			if($.isNull(vm.mallMarketCourseClassify.productId)){
				alert("请选择产品线！！！");
				return;
			}
			...
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.mallMarketCourseClassify),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
            $.ajax({
			    ...
			    success: function(r){
			    	if(r.code === 0){
						 vm.mallMarketCourseClassify = r.data;
					}else{
						alert(r.msg);
					}
				}
		...
		reload: function (e , p) {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
				postData : {
                	"classifyName" : vm.q.classifyName,
		...
		selectProduct : function(){
			productLay.show(function(index,opt){
				vm.mallMarketCourseClassify.productId = opt.productId;
				vm.mallMarketCourseClassify.productName = opt.productName;
			});
		}
	}
});