...
var csKnowleTreeLay = {
	init : function(){
		var that = this;
		$("#csKnowleTreeLay_jqGridX").jqGrid({
		   ...
		   autowidth:false,
		   multiselect: false,
		   pager: "#csKnowleTreeLay_jqGridPagerX",
		   jsonReader : {
			   root: "data.list",
			   page: "data.currPage",
		   ...
		   gridComplete:function(){
			   //隐藏grid底部滚动条
			   $("#csKnowleTreeLay_jqGridPagerX").closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "hidden" }); 
		   }
		});
	},
	courseId :"",//课程ID
	...
	layerHeigh : '500px',
	layerWeigh : '1100px',
	show : function(courseId , scallback,knowledgeId){
		var that = this;
		that.courseId = courseId;
		that.scallback = scallback;
		if(knowledgeId){
            that.knowledgeId = knowledgeId;
		}
		//树
...
//			area : "auto",
	...
	reload:function(){//加载
		var that = this;
		//知识点名称
		var csKnowleTreeLay_knowleName = $("#csKnowleTreeLay_knowleName").val()
		//知识点编号
		var csKnowleTreeLay_knowleNo = $("#csKnowleTreeLay_knowleNo").val()
		$("#csKnowleTreeLay_jqGridX").jqGrid('setGridParam',{ 
			postData : {
				knowledgeName : csKnowleTreeLay_knowleName,
				knowledgeNo : csKnowleTreeLay_knowleNo,
	...
	select:function(){//选择
		var selectDetail = getJqGridSelectedRow("csKnowleTreeLay_jqGridX");
		if(selectDetail == null){
			return ;
		}
		//行数据
		var rowData = $("#csKnowleTreeLay_jqGridX").jqGrid('getRowData',selectDetail);
	...
	reloadCouresTree: function() {
		...
		hq.ajax({
			type: "GET",
		    url: "../courses/layer/courseTree/" + that.courseId,
		    success: function(r){
				if(r.code == 0){
					that.menu_setting.callback.onClick = that.zTreeOnClick;
					that.menu_ztree = $.fn.zTree.init($("#csKnowleTreeLay_Tree"), that.menu_setting, r.data);
