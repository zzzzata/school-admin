...
var vm = new Vue({
    ...
    methods: {
        ...
        subjectStatsticsLayShow : function(){//团队
            subjectStatsticsLay.show(function(index,opt){
                // localStorage.clear();
                // vm.qCourseAbnormalOrder.classTeacherId = opt.userId;
                $('#query-teamName').val(opt.name);
                localStorage.setItem("teamId",opt.id);
                localStorage.setItem("level",opt.level);
                localStorage.setItem("teamParentId",opt.parentId);
            } , 2);
        },


        headTeacherShow : function(){//班主任
            var teamValue = $('#query-teamName').val();
        ...
        classLayerShow : function(){//班级浮层
            var teacherName = $('#query-headTeacherName').val();
        ...
        clearQcousrAbnormalOrder:function () {
            localStorage.clear();
            // $('#excelIframe').attr('src',"http://report.hqjy.com/rdppage/main/bef83ff87bf1a2120fbda90080ba0942?studentStatus=0");
...
function getStatisticExcel(){
    ...
    var studentStatus = $("#studentStatus").find("option:selected").val();
    var minAttendPer = $('#attendRateStart').val();
    var maxAttendPer  = $('#attendRateEnd').val();
    ...
    var sysUserId = localStorage.getItem("userId");
    var classId =  localStorage.getItem("classId");
    // var startDayTime = $('#qStartTime').val();
    var endDayTime = $('#qEndTime').val();
    // var url = $('#excelIframe').attr('src')+"?studentStatus="+studentStatus;
    var url = "http://report.hqjy.com/rdppage/main/bef83ff87bf1a2120fbda90080ba0942?studentStatus="+studentStatus;
    if(teamId!=null){
        url = url +"&teamId="+teamId;
    }
    if(sysUserId!=null){
        url = url + "&sysUserId="+sysUserId;
    }
    if(classId!=null){
    ...
    if(classplanIds!=null && classplanIds!=undefined && classplanIds.length>0){
        url = url +"&classplanId="+classplanIds;
    }else{
        alert("排课计划为必选项!");
        return;
    }
    if(endDayTime.trim().length>0){
        endDayTime = endDayTime +" 23:59:59";
        url = url + "&endDayTime="+endDayTime;
    }else if(endDayTime==null || endDayTime.trim().length==0){
        alert("结束时间为必选项");
        return;
    }

    if((isNumber(minAttendPer) && isNumber(maxAttendPer))
        ||(!isNumber(minAttendPer) && !isNumber(maxAttendPer) && minAttendPer=="" && maxAttendPer=="")){
        if(isNumber(minAttendPer)){
            minAttendPer = (minAttendPer/100).toFixed(2);
            maxAttendPer = (maxAttendPer/100).toFixed(2);
            url = url + "&minAttendPer="+minAttendPer+"&maxAttendPer="+maxAttendPer;
        }
    }else if(!isNumber(minAttendPer) || !isNumber(maxAttendPer)){
        alert("出勤率必须是数字!");
        return;
    }else{
        alert("出勤率必须是数字!");
    ...
    url = url +"&groupByField=classId,classplanId"
...
function initIframe() {
    // var oIframe = window.top.document.getElementById("369a90dd15cbef4943678af620ba1034");
