...
var goodsInfoLay = {
	init : function(){
		...
		$("#goodsInfoLay_jqGrid").jqGrid({
		   ...
		   autowidth:true,
		   multiselect: false,
		   pager: "#goodsInfoLay_jqGridPager",
		   jsonReader : {
			   root: "data.list",
			   page: "data.currPage",
		   ...
		   gridComplete:function(){
			   //隐藏grid底部滚动条
			   $("#goodsInfoLay_jqGridPager").closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "hidden" }); 
		   }
			});
	},
	scallback : null,//回调方法
...
//	layerWeigh : '790px',
	...
	reload:function(){
		$("#goodsInfoLay_jqGrid").jqGrid('setGridParam',{ 
			postData : {
	...
	select:function(){
		var selectDetail = getJqGridSelectedRow("goodsInfoLay_jqGrid");
		if(selectDetail == null){
			return ;
		}
		//行数据
		var rowData = $("#goodsInfoLay_jqGrid").jqGrid('getRowData',selectDetail);
