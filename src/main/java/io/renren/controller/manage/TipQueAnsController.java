 ...
 */
@Controller
@RequestMapping("/tipQueAns")
public class TipQueAnsController extends AbstractController {
    ...
    @ResponseBody
    @RequestMapping("/list")
    @RequiresPermissions("tipQueAns:list")
    public R list(HttpServletRequest request) throws Exception {
        ...
        String result = HttpClientUtil4_3.post(KUAIDA_API+"/api/accountQueAns/tipTopic",params,null);
        logger.info("url："+KUAIDA_API+"/api/accountQueAns/tipTopic");
        logger.info("params："+params.toString());
        logger.info("result："+result);
        List<TipQueAns> list = null;
        if(StringUtils.isNotBlank(result)) {
            JSONObject object = JSONObject.parseObject(result);
            if(null != object && 1 == object.getIntValue("code")) {
                list = JSONObject.parseArray(object.getString("data"),TipQueAns.class);
            }
